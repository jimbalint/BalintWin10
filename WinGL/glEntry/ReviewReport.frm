...
Begin VB.Form ReviewReport 
   Caption         =   "Review Report"
   ClientHeight    =   3165
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   4950
   Icon            =   "ReviewReport.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   3165
   ScaleWidth      =   4950
   ...
   End
End
Attribute VB_Name = "ReviewReport"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Dim credits, debits As Currency
...
Private Sub Form_Load()
    Dim scr, SQL As String
    vsr.Load "\aavb\Reports\Reports.xml", "General Ledger Data Entry Review"
    SQL = "SELECT glhistory.account, glhistory.description, amount, reference, glaccount.description"
    SQL = SQL & " FROM GLHistory LEFT JOIN GLAccount ON GLHistory.account = GLAccount.Account"
    SQL = SQL & " WHERE BatchNumber=" & CStr(BatchNumber)
    vsr.DataSource.RecordSource = SQL
    vsr.Render vp
End Sub

Private Sub Form_Resize()
    vp.Height = ReviewReport.Height * 0.95
    vp.Width = ReviewReport.Width * 0.98
End Sub
