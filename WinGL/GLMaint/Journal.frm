...
Begin VB.Form frmJournal 
   Caption         =   "General Ledger - Journal Descriptions"
   ClientHeight    =   7380
   ClientLeft      =   165
   ClientTop       =   450
   ClientWidth     =   10575
...
Private Sub Form_Load()
    ...
    ' get rid of nulls
    rsInit "DELETE * FROM GLJournal WHERE Isnull(JournalSource)", cn, mrs
    rsInit "DELETE * FROM GLJournal WHERE JournalSource = 0", cn, mrs
    
    ' set the constants for the file
    dbFileName = "GLJournal"
    dbFields(0) = "JournalSource"
    dbFields(1) = "JournalName"
    dbSortCol = 0
    dbSortDesc = False
