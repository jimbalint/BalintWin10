...
Dim FName, Fext, FNameNoExt As String
Dim Flg As Boolean
...
'   On Error GoTo ConnectErr
    ...
    cnSys.ConnectionString = DriveLetter & "\balint\data\GLSystem" & Fext
    cnSys.Open
    GetCompany
    ...
    cn.ConnectionString = NoExt(FName) & Fext
    ...
    MsgBox "Connect to " & FName & " complete" & vbCrLf & vbCrLf & _
          "Press Alt-G to refresh or if the database changes", vbInformation
...
Private Sub GetCompany()
    ...
    Do Until rsCompany.EOF
        ...
        nm1 = NoExt(Mid(StrConv(rsCompany!FileName, vbLowerCase), 3, Len(rsCompany!FileName) - 2))
        nm2 = NoExt(Mid(StrConv(FName, vbLowerCase), 3, Len(FName) - 2))
        If LCase(nm1) = LCase(nm2) Then
...
Private Function NoExt(ByVal fnm As String) As String
    Dim spos, ccnt As Integer
    ccnt = 0
    For spos = Len(fnm) To 1 Step -1
        ccnt = ccnt + 1
        If Mid(fnm, spos, 1) = "." Then Exit For
    Next spos
    If spos = 1 Then
        NoExt = fnm
    Else
