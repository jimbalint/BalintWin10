...
Begin VB.Form frmBatchList 
   ...
   Begin VB.CommandButton cmdCustom4 
      Caption         =   "Custom 4"
      Height          =   615
      Left            =   9600
      TabIndex        =   18
      ...
      Width           =   1695
   End
   Begin VB.CommandButton cmdCustom2 
      Caption         =   "Custom 2"
      Height          =   615
      Left            =   9600
      TabIndex        =   17
      ...
      Width           =   1695
   End
   Begin VB.CommandButton cmdCustom3 
      Caption         =   "Custom 3"
      Height          =   615
      Left            =   9600
      TabIndex        =   16
      ...
      Width           =   1695
   End
   Begin VB.CommandButton cmdCustom1 
      Caption         =   "Custom 1"
      Height          =   615
      Left            =   9600
      TabIndex        =   15
...
Private Sub Form_Load()
    ...
    SQLString = "DELETE * FROM PRBatch WHERE IsNull(CreateDate)"
    On Error Resume Next        ' 11/06/2010 - move on if error
    ...
    With Me
        ...
        ' cmdCustom(4,3) - 4 = custom buttons 1 thru 4
        '                  3 - 1=Button Caption / 2=EXE Name / 3=ProgName
    
        ' Hernandez Construction
        SQLString = "SELECT * FROM PRGlobal WHERE TypeCode = " & PREquate.GlobalTypeCompanyOption & _
                    " AND Var1 = 'yes' " & _
                    " AND Description = 'hernandez' " & _
                    " AND Var2 = '" & GLCompany.ID & "'"
        i = InStr(1, LCase(PRCompany.Name), "hernandez", vbTextCompare)
        
        If PRGlobal.GetBySQL(SQLString) = True Or i > 0 Then
            
            .cmdEntryForm.Visible = False
            
            cmdCustom(1, 1) = "Simple IRA Report"
            cmdCustom(1, 2) = "PRGReps"
            cmdCustom(1, 3) = "ItemDetail"
            
            cmdCustom(2, 1) = "Update to QB"
            cmdCustom(2, 2) = "PRQBFunc"
            cmdCustom(2, 3) = "TaxPay"
        ...
        SQLString = "SELECT * FROM PRGlobal WHERE TypeCode = " & PREquate.GlobalTypeCompanyOption & _
                    " AND Var1 = 'yes' " & _
                    " AND Description = 'executive' " & _
                    " AND Var2 = '" & GLCompany.ID & "'"
        i = InStr(1, LCase(PRCompany.Name), "executive", vbTextCompare)
        
        If PRGlobal.GetBySQL(SQLString) = True Or i > 0 Then
        
            cmdCustom(1, 1) = "Update Net Pay to QB"
            cmdCustom(1, 2) = "PREntry"
            cmdCustom(1, 3) = "NetPay"
            
            ' 11/06/2010 - full QB update for Exec Temps
            cmdCustom(1, 1) = "Update to QB"
            cmdCustom(1, 2) = "PRQBFunc"
            cmdCustom(1, 3) = "TaxPay"
            
            cmdCustom(2, 1) = "Update Invoices to QB"
            cmdCustom(2, 2) = "PREntry"
            cmdCustom(2, 3) = "Invoice"
        ...
        SQLString = "SELECT * FROM PRGlobal WHERE TypeCode = " & PREquate.GlobalTypeCompanyOption & _
                    " AND Var1 = 'yes' " & _
                    " AND Description = 'summit orth' " & _
                    " AND Var2 = '" & GLCompany.ID & "'"
        i = InStr(1, LCase(PRCompany.Name), "summit orth", vbTextCompare)
        ...
        If cmdCustom(1, 1) = "" Then
            .cmdCustom1.Visible = False
        Else
            .cmdCustom1.Caption = cmdCustom(1, 1)
        End If
        
        If cmdCustom(2, 1) = "" Then
            .cmdCustom2.Visible = False
        Else
            .cmdCustom2.Caption = cmdCustom(2, 1)
        End If
        
        If cmdCustom(3, 1) = "" Then
            .cmdCustom3.Visible = False
        Else
            .cmdCustom3.Caption = cmdCustom(3, 1)
        End If
        
        If cmdCustom(4, 1) = "" Then
            .cmdCustom4.Visible = False
        Else
            .cmdCustom4.Caption = cmdCustom(4, 1)
        End If
...
Private Sub cmdCustom1_Click()
    CustomCall 1
End Sub
Private Sub cmdCustom2_Click()
    CustomCall 2
End Sub
Private Sub cmdCustom3_Click()
    CustomCall 3
End Sub
Private Sub cmdCustom4_Click()
    CustomCall 4
End Sub

Private Sub CustomCall(ByVal byt As Byte)
    If cmdCustom(byt, 2) = "PREntry" Then
        BatchID = fg.TextMatrix(fg.Row, 0)
        Select Case cmdCustom(byt, 3)
            Case "NetPay":          frmQBCheckUpdate.Show vbModal
            Case "Invoice":         frmQBInvUpdate.Show vbModal
        End Select
    Else
        ReportRun cmdCustom(byt, 2), cmdCustom(byt, 3)
    End If
End Sub
...
Dim TID As Double
    ...
    ' store which menu to come back to
    MenuName = BackName
    ...
    If BalintFolder = "" Then
        ShellString = Mid(App.Path, 1, 3) & "Balint\" & Trim(EXEName) & ".exe" & _
             ...
             " MenuName=" & MenuName
    Else
        ShellString = "c:\Balint\" & Trim(EXEName) & ".exe" & _
            " ProgName=" & ProgName & _
            " SysFile=" & BalintFolder & "\Data\GLSystem.mdb" & _
            " UserID=" & User.ID & _
            " Batch=" & fg.TextMatrix(fg.Row, 0) & _
            " ProgName=" & ProgName & _
            " BackName=c:\Balint\" & "\PREntry.exe" & _
            " BalintFolder=" & BalintFolder & _
            " MenuName=" & MenuName
    End If
...
'        frmEntryTS.Show vbModal
        ...
        frmEntryDPT.StartCheckNumber = frmNewBatch.tdbIntStartCheck
        frmEntryDPT.BatchID = fg.TextMatrix(fg.Row, 0)
        frmEntryDPT.Show vbModal
...
Dim PRBilling As Boolean
    ...
    ' job refresh
    Dim KPFlag As Boolean
    KPFlag = False
    If InStr(1, LCase(PRCompany.Name), "kent", vbTextCompare) Then KPFlag = True
    If InStr(1, LCase(PRCompany.Name), "battaglia", vbTextCompare) Then KPFlag = True
    
    ' turn off for dept entry clients
    ...
    If PRGlobal.GetBySQL(SQLString) = True Then
        ...
        ' create PRTimeSheet table if necessary 2016-01-16
        If TableExists("PRTimeSheet", cn) = False Then
            PRTimeSheetCreate
        End If
        
        KPFlag = True
        SQLString = "DELETE * FROM PRTimeSheet"
        cn.Execute SQLString
    
    End If
    
    If TableExists("JCJob", cn) = True And KPFlag = False Then
...
'        frmEntryTS.Show vbModal
        ...
        frmEntryDPT.StartCheckNumber = frmNewBatch.tdbIntStartCheck
        frmEntryDPT.BatchID = rs!BatchID
        frmEntryDPT.Show vbModal
